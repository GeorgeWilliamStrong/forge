...
def laplacian_10th_order(u, dx_sq, b, kernel='Taylor'):
    """
    10th order accurate laplacian.

    :param u: wavefield
    :param dx_sq: squared spatial increment
    :param b: buoyancy model (place-holder)
    :param kernel: finite difference stencil coefficients
    :return: laplacian of u
    """
    
    if kernel == 'High':
    	print('forge-private feature')
    elif kernel == 'Taylor':
        a5 = 8/25200
        a4 = 125/25200
        a3 = 1000/25200
        ...
        a1 = 42000/25200
        a0 = 73766/25200
    return (1/dx_sq)*(a5*u[:, :-10,5:-5]-a4*u[:, 1:-9,5:-5]+a3*u[:, 2:-8,5:-5]-a2*u[:, 3:-7,5:-5]+a1*u[:, 4:-6,5:-5]-a0*u[:, 5:-5,5:-5]+a1*u[:, 6:-4,5:-5]-a2*u[:, 7:-3,5:-5]+a3*u[:, 8:-2,5:-5]-a4*u[:, 9:-1,5:-5]+a5*u[:, 10:,5:-5]+a5*u[:, 5:-5,:-10]-a4*u[:, 5:-5,1:-9]+a3*u[:, 5:-5,2:-8]-a2*u[:, 5:-5,3:-7]+a1*u[:, 5:-5,4:-6]-a0*u[:, 5:-5,5:-5]+a1*u[:, 5:-5,6:-4]-a2*u[:, 5:-5,7:-3]+a3*u[:, 5:-5,8:-2]-a4*u[:, 5:-5,9:-1]+a5*u[:, 5:-5,10:])


def laplacian_10th_order_density(u, dx_sq, b, kernel='Taylor'):
    """
    10th order accurate variable density laplacian.

    :param u: wavefield
    :param dx_sq: squared spatial increment
    :param b: buoyancy model
    :param kernel: finite difference stencil coefficients
    :return: variable density laplacian of u
    """
